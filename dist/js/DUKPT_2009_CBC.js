function createDataKeyHex(ipek, ksn) {
    const derivedPEK = deriveKeyHex(ipek, ksn);
    ...
    let maskedPEK = XORdataHex(variantMask, derivedPEK); // apply mask

    maskedPEK = hexstringToData(maskedPEK);
...
function createPINKeyHex(ipek, ksn) {
    const derivedPEK = deriveKeyHex(ipek, ksn); // derive DUKPT basis key
    const variantMask = '00000000000000FF00000000000000FF'; // PIN variant
    return XORdataHex(variantMask, derivedPEK); // apply mask
}
...
function deriveKeyHex(ipek, ksn) {
    const binipek = hexstringToData(ipek);
    const binksn = hexstringToData(ksn);

    const dk = deriveKey(binipek, binksn);
...
function deriveKey(ipek, ksnString) {
    let ksn = ksnString;
    if (ksn.length === 10) {
        ksn = ksn.substring(2);
    } // we want the bottom 8 bytes

    let baseKSN = ANDdata(hexstringToData('FFFFFFFFFFE00000'), ksn);
    let curKey = ipek;
    const counter = getCounter(ksn);
...
function generateKey(key, ksn) {
    ...
    const maskedKey = XORdata(hexstringToData(mask), key);

    const left = encryptRegister(maskedKey, ksn);
    const right = encryptRegister(key, ksn);
...
function encryptRegister(key, reg) {
    ...
    const bottom8xorKSN = XORdata(bottom8, reg);

    // This will be single-DES because of the 8-byte key:
    const desEncrypted = des(
        top8,
        bottom8xorKSN,
        true, /* encrypt */
        CBC,
    ...
    return XORdata(bottom8, desEncrypted);
}
...
function encryptTDES(key, data) {
    key = EDE3KeyExpand(key);
    const CBC = 1; // cipher block chaining enabled
    const iv = '\0\0\0\0\0\0\0\0'; // initial vector

    // convert to binary
    const binaryKey = hexstringToData(key);
    let binaryData = hexstringToData(data);
...
function XORdataHex(d1, d2) {
    let data1 = hexstringToData(d1);
    let data2 = hexstringToData(d2);
...
function createIPEK(bdk, ksn) {
    ...
    let key = EDE3KeyExpand(bdk); // make 24-byte key
    key = hexstringToData(key); // make it binary

    let maskedKSN = ANDdata(
    ...
    let IPEK = dataToHexstring(cipher);
